...
function SpoolsErrandApp() {
  ...
  const [filterExtendedPlay, setFilterExtendedPlay] = useState('');
  const [filterWordRoll, setFilterWordRoll] = useState('');
  const [sortField, setSortField] = useState('title');
  const [sortDirection, setSortDirection] = useState('asc');
  const [showStats, setShowStats] = useState(false);
  const [showFilters, setShowFilters] = useState(false);
  const [duplicateWarning, setDuplicateWarning] = useState(null);
  const [backendStatus, setBackendStatus] = useState({ connected: false, rollCount: 0 });
  const [backendManufacturers, setBackendManufacturers] = useState([]);
  ...
  useEffect(() => {
    ...
    if (stored) {
      try { setRolls(JSON.parse(stored)); } catch (e) {}
    }
    checkBackend();
  }, []);

  const saveRolls = (newRolls) => {
    setRolls(newRolls);
  ...
  const checkBackend = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setBackendStatus({ connected: true, rollCount: data.rollCount });
        const mfgRes = await fetch('/api/manufacturers');
        if (mfgRes.ok) {
          const mfgData = await mfgRes.json();
          setBackendManufacturers(mfgData.manufacturers.map(m => m.manufacturer));
        }
      }
    } catch (e) {
      setBackendStatus({ connected: false, rollCount: 0 });
    }
  };
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      if (data.found && data.results.length > 0) {
        ...
        alert('No matches found in the catalog for ' + formData.manufacturer + ' #' + formData.rollNumber + '.');
      }
    } catch (e) {
  ...
  const applyLookupResult = (r) => {
    setFormData({
      ...formData,
      title: r.title || formData.title,
      artist: r.artist || formData.artist,
      composer: r.composer || formData.composer,
      year: r.year || formData.year,
  ...
  const triggerSuggest = useCallback((field, value) => {
  ...
  const applySuggestion = (s) => {
    setFormData({
      ...
      title: s.title || formData.title,
      artist: s.artist || formData.artist,
      composer: s.composer || formData.composer,
      year: s.year || formData.year,
  ...
  const handleFieldChange = (field, value) => {
    ...
    if (['title', 'artist', 'composer'].includes(field)) {
      triggerSuggest(field, value);
    }
  };

  const handleSubmit = () => {
    if (!formData.manufacturer || !formData.rollNumber || !formData.title) {
      alert('Manufacturer, Roll Number, and Title are required.');
      return;
    }
    if (!editingId) {
    ...
    let newRolls;
    if (editingId) {
      newRolls = rolls.map(r => r.id === editingId ? { ...formData, id: editingId } : r);
    } else {
      newRolls = [...rolls, { ...formData, id: Date.now().toString() }];
    }
    saveRolls(newRolls);
    resetForm();
  };
  ...
  const handleEdit = (roll) => { setFormData(roll); setEditingId(roll.id); setShowForm(true); };
  const handleDelete = (id) => {
    if (confirm('Delete this roll from your collection?')) {
      saveRolls(rolls.filter(r => r.id !== id));
    }
  };
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (Array.isArray(imported)) {
          if (confirm('Import ' + imported.length + ' rolls? This will add to your existing collection.')) {
            saveRolls([...rolls, ...imported.map(r => ({ ...r, id: r.id || Date.now().toString() + Math.random() }))]);
          }
        }
  ...
  const filteredAndSorted = useMemo(() => {
    let result = rolls.filter(r => {
      ...
      const matchesSearch = !q || [r.manufacturer, r.rollNumber, r.title, r.artist, r.composer, r.catalogSeries, r.notes, r.storageLocation].some(f => String(f || '').toLowerCase().includes(q));
      const matchesMfg = !filterManufacturer || r.manufacturer === filterManufacturer;
      const matchesType = !filterRollType || r.rollType === filterRollType;
      const matchesEP = filterExtendedPlay === '' || r.extendedPlay === (filterExtendedPlay === 'yes');
      const matchesWord = filterWordRoll === '' || r.wordRoll === (filterWordRoll === 'yes');
      return matchesSearch && matchesMfg && matchesType && matchesEP && matchesWord;
    });
    result.sort((a, b) => {
    ...
    return result;
  }, [rolls, searchQuery, filterManufacturer, filterRollType, filterExtendedPlay, filterWordRoll, sortField, sortDirection]);

  const stats = useMemo(() => {
    const totalRolls = rolls.reduce((sum, r) => sum + parseInt(r.quantity || 1), 0);
  ...
  const manufacturerOptions = useMemo(() => {
    const fromColl = [...new Set(rolls.map(r => r.manufacturer).filter(Boolean))];
    return [...new Set([...backendManufacturers, ...fromColl])].sort();
  }, [backendManufacturers, rolls]);
  ...
  const SuggestionsDropdown = ({ suggestions, onPick, onClose }) => (
    <div className="absolute z-20 mt-1 w-full rounded border shadow-lg max-h-64 overflow-y-auto"
      ...
      <button onClick={onClose} className="block w-full text-center p-1 text-xs italic" style={{ color: '#8b5a2b' }}>Close suggestions</button>
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs" style={{ color: '#8b5a2b' }}>
              {backendStatus.connected
                ? '● Catalog: ' + backendStatus.rollCount.toLocaleString() + ' rolls'
                : '○ Backend offline'}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <button onClick={() => { resetForm(); setShowForm(true); }}
                  className="px-3 py-2 rounded text-sm font-semibold text-white hover:opacity-90"
                  style={{ backgroundColor: '#8b5a2b' }}>+ Add Roll</button>
          <button onClick={() => setShowStats(!showStats)}
                  className="px-3 py-2 rounded text-sm font-semibold hover:opacity-90"
                  style={{ backgroundColor: '#e8d9b8', color: '#5c3a1e' }}>{showStats ? 'Hide' : 'Show'} Stats</button>
          <button onClick={() => setShowFilters(!showFilters)}
                  className="px-3 py-2 rounded text-sm font-semibold hover:opacity-90"
                  style={{ backgroundColor: '#e8d9b8', color: '#5c3a1e' }}>Filters</button>
          <button onClick={handleExportJSON} disabled={rolls.length === 0}
                  className="px-3 py-2 rounded text-sm font-semibold hover:opacity-90 disabled:opacity-50"
                  style={{ backgroundColor: '#e8d9b8', color: '#5c3a1e' }}>Export JSON</button>
          <button onClick={handleExportCSV} disabled={rolls.length === 0}
                  className="px-3 py-2 rounded text-sm font-semibold hover:opacity-90 disabled:opacity-50"
                  style={{ backgroundColor: '#e8d9b8', color: '#5c3a1e' }}>Export CSV</button>
          <label className="px-3 py-2 rounded text-sm font-semibold hover:opacity-90 cursor-pointer"
                 style={{ backgroundColor: '#e8d9b8', color: '#5c3a1e' }}>
        ...
        {showStats && (
          <div className="mb-4 p-4 rounded border" style={{ backgroundColor: '#faf3e3', borderColor: '#8b5a2b' }}>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
                <h3 className="font-semibold mb-1" style={{ color: '#5c3a1e' }}>By Manufacturer</h3>
                {Object.entries(stats.byMfg).sort((a,b) => b[1]-a[1]).map(([m,c]) => (
                  <div key={m} className="flex justify-between border-b py-1 text-sm" style={{ borderColor: '#e8d9b8', color: '#5c3a1e' }}>
                    <span>{m}</span><span className="font-semibold">{c}</span>
                  </div>
                ))}
              </div>
              <div>
                <h3 className="font-semibold mb-1" style={{ color: '#5c3a1e' }}>By Roll Type</h3>
                {Object.entries(stats.byType).sort((a,b) => b[1]-a[1]).map(([t,c]) => (
                  <div key={t} className="flex justify-between border-b py-1 text-sm" style={{ borderColor: '#e8d9b8', color: '#5c3a1e' }}>
        ...
        <div className="mb-4 p-3 rounded border" style={{ backgroundColor: '#faf3e3', borderColor: '#8b5a2b' }}>
          ...
          {showFilters && (
            <div className="grid grid-cols-4 gap-2 mt-2">
              ...
              <select value={filterExtendedPlay} onChange={(e) => setFilterExtendedPlay(e.target.value)} className="px-2 py-1 rounded border text-sm" style={{ borderColor: '#8b5a2b' }}>
                <option value="">EP: Any</option><option value="yes">EP: Yes</option><option value="no">EP: No</option>
              </select>
              <select value={filterWordRoll} onChange={(e) => setFilterWordRoll(e.target.value)} className="px-2 py-1 rounded border text-sm" style={{ borderColor: '#8b5a2b' }}>
                <option value="">Word: Any</option><option value="yes">Word: Yes</option><option value="no">Word: No</option>
              </select>
            </div>
          )}
        </div>
        ...
        <div className="rounded border overflow-x-auto" style={{ backgroundColor: '#fff', borderColor: '#8b5a2b' }}>
          {filteredAndSorted.length === 0 ? (
            <div className="p-8 text-center" style={{ color: '#8b5a2b' }}>
              {rolls.length === 0
                ? <><p className="text-lg">Your collection is empty.</p><p className="text-sm">Click "Add Roll" to catalog your first piano roll.</p></>
                : <p>No rolls match your search or filters.</p>}
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead style={{ backgroundColor: '#f5ecd9' }}>
                <tr>
                  ...
                  <SortHeader field="rollNumber">Roll #</SortHeader>
                  <SortHeader field="title">Title</SortHeader>
                  <SortHeader field="artist">Artist</SortHeader>
                  <SortHeader field="composer">Composer</SortHeader>
                  <SortHeader field="rollType">Type</SortHeader>
                  <SortHeader field="condition">Condition</SortHeader>
                  <SortHeader field="quantity">Qty</SortHeader>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#5c3a1e' }}>Flags</th>
                  <th className="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wider" style={{ color: '#5c3a1e' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredAndSorted.map((roll, idx) => (
                  <tr key={roll.id} style={{ backgroundColor: idx % 2 === 0 ? '#fff' : '#faf3e3', color: '#5c3a1e' }}>
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      <button onClick={() => handleEdit(roll)} className="p-1 hover:opacity-70">✎</button>
                      <button onClick={() => handleDelete(roll.id)} className="p-1 hover:opacity-70 ml-1">🗑</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-2 text-xs text-right italic" style={{ color: '#8b5a2b' }}>
          Showing {filteredAndSorted.length} of {rolls.length} roll{rolls.length !== 1 ? 's' : ''}
        </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ backgroundColor: 'rgba(92, 58, 30, 0.6)' }}>
            <div className="rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto" style={{ backgroundColor: '#faf3e3' }}>
              <div className="p-4 border-b flex justify-between items-center sticky top-0" style={{ backgroundColor: '#faf3e3', borderColor: '#8b5a2b' }}>
                ...
                <button onClick={resetForm} style={{ color: '#5c3a1e' }}>✕</button>
              </div>

              {lookupResults && (
                <div className="m-4 p-3 rounded border-2" style={{ borderColor: '#5c8b2b', backgroundColor: '#f4f9e7' }}>
                  <p className="font-semibold mb-2" style={{ color: '#5c3a1e' }}>Found {lookupResults.length} match{lookupResults.length !== 1 ? 'es' : ''} in catalog</p>
                  <div className="space-y-1">
                    {lookupResults.map((r, i) => (
                      <button key={i} onClick={() => applyLookupResult(r)}
                              className="block w-full text-left p-2 rounded text-sm hover:opacity-80"
                              style={{ backgroundColor: '#fff', color: '#5c3a1e' }}>
              ...
              {duplicateWarning && (
                <div className="m-4 p-3 rounded border-2" style={{ borderColor: '#8b5a2b', backgroundColor: '#fff8e1' }}>
                  ...
                  <p className="text-sm" style={{ color: '#5c3a1e' }}>You already have "{duplicateWarning.title}" ({duplicateWarning.manufacturer} #{duplicateWarning.rollNumber}) with quantity {duplicateWarning.quantity}.</p>
                  <div className="flex gap-2 mt-2">
                    <button onClick={incrementDuplicate} className="px-3 py-1 rounded text-sm text-white" style={{ backgroundColor: '#8b5a2b' }}>Increment Quantity</button>
                    <button onClick={() => setDuplicateWarning(null)} className="px-3 py-1 rounded text-sm" style={{ backgroundColor: '#e8d9b8', color: '#5c3a1e' }}>Add Anyway</button>
                  </div>
                </div>
              )}
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                <div>
                  ...
                  <datalist id="mfg-list">{manufacturerOptions.map(m => <option key={m} value={m} />)}</datalist>
                </div>
                <div>
                  ...
                  <div className="flex gap-1">
                    ...
                    <button onClick={handleLookup} disabled={!backendStatus.connected || lookupLoading} className="px-2 py-1 rounded text-xs font-semibold text-white disabled:opacity-50" style={{ backgroundColor: '#5c8b2b' }}>{lookupLoading ? '...' : 'Lookup'}</button>
                  </div>
                </div>
                <div className="col-span-2 relative">
                  <label className="block text-sm font-semibold mb-1" style={{ color: '#5c3a1e' }}>Title *</label>
                  <input type="text" value={formData.title} onChange={(e) => handleFieldChange('title', e.target.value)} className="w-full px-2 py-1 rounded border" style={{ borderColor: '#8b5a2b' }} />
                  {activeSuggestField === 'title' && searchSuggestions.length > 0 && <SuggestionsDropdown suggestions={searchSuggestions} onPick={applySuggestion} onClose={() => { setSearchSuggestions([]); setActiveSuggestField(null); }} />}
                </div>
                <div className="relative">
                  <label className="block text-sm font-semibold mb-1" style={{ color: '#5c3a1e' }}>Artist</label>
                  <input type="text" value={formData.artist} onChange={(e) => handleFieldChange('artist', e.target.value)} className="w-full px-2 py-1 rounded border" style={{ borderColor: '#8b5a2b' }} />
                  {activeSuggestField === 'artist' && searchSuggestions.length > 0 && <SuggestionsDropdown suggestions={searchSuggestions} onPick={applySuggestion} onClose={() => { setSearchSuggestions([]); setActiveSuggestField(null); }} />}
                </div>
                <div className="relative">
                  <label className="block text-sm font-semibold mb-1" style={{ color: '#5c3a1e' }}>Composer</label>
                  <input type="text" value={formData.composer} onChange={(e) => handleFieldChange('composer', e.target.value)} className="w-full px-2 py-1 rounded border" style={{ borderColor: '#8b5a2b' }} />
                  {activeSuggestField === 'composer' && searchSuggestions.length > 0 && <SuggestionsDropdown suggestions={searchSuggestions} onPick={applySuggestion} onClose={() => { setSearchSuggestions([]); setActiveSuggestField(null); }} />}
                </div>
              ...
              <div className="p-4 border-t flex justify-end gap-2 sticky bottom-0" style={{ backgroundColor: '#faf3e3', borderColor: '#8b5a2b' }}>
                <button onClick={resetForm} className="px-4 py-2 rounded font-semibold" style={{ backgroundColor: '#e8d9b8', color: '#5c3a1e' }}>Cancel</button>
                <button onClick={handleSubmit} className="px-4 py-2 rounded font-semibold text-white" style={{ backgroundColor: '#8b5a2b' }}>{editingId ? 'Save Changes' : 'Add Roll'}</button>
              </div>
            </div>
          </div>
        )}
