...
- `./ingest.sh` — runs all ingestion pipelines (IAMMP + RPRF + QRS) against `data/sources/`.
- `npm run ingest:iammp` / `npm run ingest:rprf` / `npm run ingest:qrs` — run a single pipeline.
...
- `ingest-qrs.js` reads `qrs_catalog.pdf` (a 242-page QRS Music product catalog). `parsers/qrs-pdf.js` is section-aware: it splits the catalog into per-page blocks via "Pg N" markers and applies a different row format per page range — QRS own rolls (pp. 53–84), "Resurrected" Duo-Art Popular (86–111) / Classical (114–147), Ampico Popular+Classical (148–175), Welte (176). The Resurrected sections are stored under distinct manufacturer values like `QRS Resurrected (Duo-Art)` so they don't collide with original RPRF entries.
